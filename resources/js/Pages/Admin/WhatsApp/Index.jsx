...
function AssistantCard({ assistant }) {
  ...
  const fallbacks = assistant?.fallbacks ?? [];
  const status = ready
    ? { label: 'Ativo', tone: 'success' }
    : assistant?.enabled
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      ...
      {assistant?.enabled && (
        <p className="mt-3 text-xs text-slate-500 dark:text-gray-400">
          Principal: {assistant.provider || 'groq'} · {assistant.model || 'não configurado'}
          {fallbacks.map((fallback) => (
            <React.Fragment key={fallback.provider}>
              {' '}· Fallback: {fallback.provider} · {fallback.model || 'não configurado'}
              {fallback.configured ? '' : ' (sem chave)'}
            </React.Fragment>
          ))}{' '}
          · Limite interno: {assistant.daily_limit ?? 300} interpretações por dia. Se todos os
          provedores falharem, perguntas conhecidas usam o interpretador local.
        </p>
      )}

      {assistant?.enabled && !assistant?.configured && (
        <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
          Configure a chave gratuita da Groq ou do Gemini no servidor para ativar a interpretação
          por IA.
        </p>
      )}
    </section>
