...
const PERMISSION_LABELS = {
  view_audit_logs: {
    ...
    description: 'Visualizar o histórico de ações do sistema.',
  },
  manage_admins: {
    label: 'Gerenciar administradores',
    description: 'Criar, editar e excluir administradores.',
  },
  manage_employees: {
    label: 'Gerenciar funcionários',
    description: 'Criar, editar e excluir outros funcionários e suas permissões.',
  },
  manage_whatsapp: {
    label: 'Gerenciar integração WhatsApp',
    description: 'Conectar, atualizar ou remover o WhatsApp da empresa.',
  },
  create_timeslots_via_whatsapp: {
...
function Avatar({ name }) {
  ...
  return (
    <span
      className={`inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold ${color}`}
    >
      {initials}
    </span>
  );
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                    {employees.map((employee) => {
                      ...
                      return (
                        <tr
                          key={employee.id}
                          className="transition hover:bg-gray-50/70 dark:hover:bg-gray-700/40"
                        >
      ...
      >
        <div className="space-y-4">
          ...
          <FormActions>
            ...
            <Button variant="primary" className="flex-1" onClick={savePermissions}>
              Salvar permissões
            </Button>
          </FormActions>
        </div>
