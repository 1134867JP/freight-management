...
class UpdateDocaRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $docaId    = $this->route('doca')?->id;

        return [
            'nome'      => ['required', 'string', 'max:100'],
            'codigo'    => [
                'required', 'string', 'max:20',
                Rule::unique('docas')->where('company_id', $companyId)->ignore($docaId),
            ],
            'descricao' => ['nullable', 'string', 'max:500'],
            'is_active' => ['boolean'],
            'status'    => ['nullable', new Enum(DocaStatus::class), Rule::notIn([DocaStatus::Occupied->value])],
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'status.not_in'    => 'O status "Ocupada" não pode ser definido manualmente.',
        ];
    }
}
